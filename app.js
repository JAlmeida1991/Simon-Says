...
function showColor(arr) {
    ...
    function foo() {
        ...
        arr[index].style.opacity = .8;
        setTimeout(function () {
            arr[index].style.opacity = .4;
            index++;
...
function compareColors() {
    ...
    if (String.prototype.startsWith.call(colorArr.map(x => x.className), userGuess.map(x => x.className)) && colorArr.length === userGuess.length) {
        addRandomColor();
        // NEED IN ORDER TO DELAY CORRECT ANSWER
        setTimeout(function () {
            showColor(colorArr);
    ...
    } else if (!String.prototype.startsWith.call(colorArr.map(x => x.className), userGuess.map(x => x.className))) {
        setTimeout(function () {
            showColor(colorArr);
            userGuess.splice(0, colorArr.length);
        }, 1000)
